...
class PessoasForm(forms.ModelForm):
    pj = forms.BooleanField(required=False, label='Pessoa Juridica', widget=forms.CheckboxInput(attrs={'class':'form-check-input', 'id':'check_pj'},check_test=lambda x: False), initial='True',label_suffix='')
    ...
    endereco_residencial = forms.CharField(required=False, label='Endereco residencial', widget=forms.TextInput(attrs={'class':'form-control'}))
    atuacao = forms.ChoiceField(required=True,widget=forms.HiddenInput(),choices=BOP_ATU,initial='RELATOR')
    
    class Meta:
        model = ModelPessoas
        fields = ['pj','nome_completo', 'data_nascimento', 'genero', 'nro_documento','nome_mae','nome_pai','endereco_residencial']