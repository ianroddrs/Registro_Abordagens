...
def add_pessoa(request):
    if request.GET.get('q'):
        ...
        cx = {'resultados_pessoa': pessoa}
        template = "includes/resultados_pesquisa.html"
    else:
        url_parameter = request.GET.get("p")
        pessoa = ModelUsuarios.objects.filter(nome_completo__icontains=url_parameter)  
        cx = {'resultados_comando': pessoa}
        template = "includes/operacao.html"
...
def converter_datas(context):
   for chave in context:
       if isinstance(context[chave], dict):
           converter_datas(context[chave])
       elif isinstance(context[chave], list):
           for item in context[chave]:
               converter_datas(item)
       elif isinstance(context[chave], str) and context[chave].count('-') == 2:
           context[chave] = datetime.datetime.strptime(context[chave], "%Y-%m-%d")
   return context
...
def extrato(request):
   ...
   context = converter_datas(context)
   context.update({
...
def save_pessoas(request,model, registro):
    ...
    for pessoa in form_pessoas:
        ...
        if model == 'ModelBop_Pesssoa':
            atuacao = ModelBop_Atuacao.objects.get(ds_atuacao=pessoa['atuacao'].value())
            ...
            atuacao = ModelProced_Atuacao.objects.get(ds_atuacao=pessoa['atuacao'].value())
